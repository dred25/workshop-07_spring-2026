...
H~0~: **pH has no effect abalone shell growth**  
...
# base layer ggplot
ggplot(data = abalone_clean, # data from abalone clean
       aes(x = mean_ph, # x-axis is mean ph
           y = change_in_area)) +  #y-axis is change in area
  # second layer points to represent observations
  geom_point(size = 4, #point size
             stroke = 1,
             fill = "firebrick4",#change color of plot
             shape = 21)
```

Where does the missing value come from?  

**The missing value comes from the pair of tanks in which pH sensor malfunctioned**  
...
abalone_model <- lm(
  ...
  data = abalone_clean      # data fram: cleaned abalone data
)
```
...
'x', 'predicted', 'std.error', 'conf.low', 'conf.high'(not going to worry about
'group' right now)
...
It is a 'ggeffects' output and a regular data fram
...
**At ph = 8, the model predicts change in abalone shell surface area to be 5.36 
[95% CI: 5.08, 5.65] mm^2^ d^-1^.**
